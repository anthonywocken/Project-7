...
/**
 * Class MashUpFrame provides the user interface for a simple simulation
 * program.
 * 
...
public class MashUpFrame extends JFrame {
    private MashUpModel myModel;
    private MashUpPanel myPicture;
    ...
    public MashUpFrame(int width, int height) {
        ...
        setTitle("CSC142 MashUp simulation");
        setDefaultCloseOperation(EXIT_ON_CLOSE);
        myModel = new MashUpModel(width, height);

        // set up player picture panel
        myPicture = new MashUpPanel(myModel);
    ...
    // starts the simulation...assumes all Players have already been added
    public void start() {
        // don't let anyone start a second time and remember if we have started
        if (started) {
            return;
        }
        // if they didn't add any players, then nothing to do
        if (myModel.getCounts().isEmpty()) {
            System.out.println("nothing to simulate--no Players");
            return;
        }
        started = true;
        addClassCounts();
    ...
    // add right-hand column showing how many of each player are alive
    ...
    // add a certain number of players of a particular class to the simulation
    public void add(int number, Class<? extends MashUpPlayer> c) {
        // don't let anyone add players after simulation starts
        if (started) {
            return;
        }
        // temporarily turning on started flag prevents player constructors
        // from calling add
        started = true;
